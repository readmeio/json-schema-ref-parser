...
describe('json-schema-ref-parser package exports', function () {
  ...
  it('should export the $RefParser class as the default CommonJS export', async function () {
    expect(commonJSExport).to.satisfy(is$RefParser);
  });

  it('should export the $RefParser class as the default ESM export', async function () {
    expect(defaultExport).to.satisfy(is$RefParser);
  });

  it('should export the JSONParserError class as a named ESM export', async function () {
    expect(JSONParserError).to.be.a('function');
    expect(JSONParserError.name).to.equal('JSONParserError');
  });

  it('should export the InvalidPointerError class as a named ESM export', async function () {
    expect(InvalidPointerError).to.be.a('function');
    expect(InvalidPointerError.name).to.equal('InvalidPointerError');
  });

  it('should export the MissingPointerError class as a named ESM export', async function () {
    expect(MissingPointerError).to.be.a('function');
    expect(MissingPointerError.name).to.equal('MissingPointerError');
  });

  it('should export the ResolverError class as a named ESM export', async function () {
    expect(ResolverError).to.be.a('function');
    expect(ResolverError.name).to.equal('ResolverError');
  });

  it('should export the ParserError class as a named ESM export', async function () {
    expect(ParserError).to.be.a('function');
    expect(ParserError.name).to.equal('ParserError');
  });

  it('should export the UnmatchedParserError class as a named ESM export', async function () {
    expect(UnmatchedParserError).to.be.a('function');
    expect(UnmatchedParserError.name).to.equal('UnmatchedParserError');
  });

  it('should export the UnmatchedResolverError class as a named ESM export', async function () {
    expect(UnmatchedResolverError).to.be.a('function');
    expect(UnmatchedResolverError.name).to.equal('UnmatchedResolverError');
  });

  it('should not export anything else', async function () {
